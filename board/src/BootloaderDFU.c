 ...
 */
bool IsSecure      DFU_DATA = SECURE_MODE;
 ...
 */
bool WaitForExit DFU_DATA  = false;

/** Current DFU state machine state, one of the values in the DFU_State_t enum. */
uint8_t DFU_State DFU_DATA = dfuIDLE;

/** Status code of the last executed DFU command. This is set to one of the values in the DFU_Status_t enum after
 *  each operation, and returned to the host when a Get Status DFU request is issued.
 */
uint8_t DFU_Status DFU_DATA = OK;

/** Data containing the DFU command sent from the host. */
DFU_Command_t SentCommand DFU_DATA;

/** Response to the last issued Read Data DFU command. Unlike other DFU commands, the read command
 *  requires a single byte response from the bootloader containing the read data when the next DFU_UPLOAD command
 *  is issued by the host.
 */
uint8_t ResponseByte DFU_DATA;
 ...
 */
uint8_t Flash64KBPage DFU_DATA  = 0;
#endif

/** Memory start address, indicating the current address in the memory being addressed (either FLASH or EEPROM
 *  depending on the issued command from the host).
 */
uint16_t StartAddr DFU_DATA  = 0x0000;
